...
const OA_TYPE_COLORS: Record<string, string> = {
  ...
  Diamant: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300",
...
const SOURCE_COLORS: Record<string, string> = {
  openalex: "bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-300",
...
export function JournauxOaPage() {
  ...
  const { data, isLoading, isError, error } = useQuery<{
    data: JournalResult[];
    meta: { total: number; source: string; query: string; warnings?: string[] };
  }>({
    ...
    onSuccess: (content) => {
      try {
        ...
        if (jsonMatch) {
          ...
          const ranked: number[] = parsed.ranked ?? parsed.order ?? [];
          if (ranked.length > 0) {
            queryClient.setQueryData<{
              data: JournalResult[];
              meta: { total: number; source: string; query: string };
            }>(["journaux-oa", debouncedQuery, source, subject, searchTrigger], (old) => {
              if (!old) return old;
              ...
              const remaining = reordered.filter(
                (j) => !ranked.includes(reordered.indexOf(j) + 1)
              );
              return {
                ...old,
                data: [
                  ...sorted,
                  ...remaining,
                ].map((j, i) => ({ ...j, relevanceScore: j.relevanceScore + (sorted.length - i) * 1000 })),
              };
            });
        ...
        toast.info("L'IA n'a pas pu générer un classement exploitable");
      } catch {
        toast.info("Réponse IA reçue mais format inattendu");
      }
    },
    onError: () => {
            ...
            <div className="flex items-center gap-2 sm:ml-auto">
              <Select value={subject} onValueChange={setSubject}>
                ...
                <SelectContent>
                  ...
                  {SUBJECT_AREAS.filter((s) => s !== "Toutes les disciplines").map(
                    (s) => (
      ...
      {journals.length > 0 && (
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground">
            ...
            trouvé{filteredJournals.length !== 1 ? "s" : ""}
            {subject !== "all" && (
              <span className="ml-1">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              disabled={rankMutation.isPending || filteredJournals.length === 0}
            >
